...
export default function FlashcardPlayer({ module, deck, onExit }) {
  ...
  /* -----------------------------------------------------------------------
     ...
     Pointer events cover touch and mouse in one path. touch-action: pan-y on
     the card lets the page still scroll vertically while we claim horizontal
     movement for swiping.
     ----------------------------------------------------------------------- */
  ...
  function onPointerMove(e) {
    ...
    // Vertical movement means they're scrolling the page, not swiping.
  ...
  function onPointerUp() {
    ...
    if (Math.abs(dx) > SWIPE_THRESHOLD && total > 1) {
      // Let the fly-away animation carry on from where the finger left it.
    ...
    <>
      ...
      <Screen>
        ...
        {!card ? (
          ...
          <>
            {/* overflow-hidden keeps the flying card from widening the page
                and causing a horizontal scrollbar. */}
            <div className="mt-5 overflow-hidden -mx-1 px-1">
              <div
                ...
                style={cardStyle}
                className="w-full min-h-[280px] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 flex flex-col items-center justify-center text-center cursor-grab active:cursor-grabbing select-none"
              >
                {deck.cat[card.c] && (
